...
export default async function WorkSection() {
  const res = await fetch(
    "https://main-portfolio-app-backend.onrender.com/works",
    {
      next: { revalidate: 1 },
    },
  );
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch work posts: ${res.status} ${res.statusText}`,
    );
  }
            ...
            <article key={post.id} className="group">
              <div className="flex flex-col md:flex-row gap-6 md:gap-8">
                ...
                <div className="flex-1 flex flex-col justify-start">
                  <Link
                    href={`/work/${post.id}`}
                    className="text-2xl md:text-3xl font-bold text-gray-900 mb-4 group-hover:text-teal-600 transition-colors"
                  >
                    {post.title}
                  </Link>
