...
export default async function BlogSection() {
 ...
 throw new Error(`Failed to fetch blog posts: ${res.status} ${res.statusText}`)
  ...
  const blogPosts:BlogPostsResponse=await res.json()
  ...
  />
                ...
                <div className="flex-1 flex flex-col justify-start">
                  <Link href={`/blog/${post.id}`} className="text-2xl md:text-3xl font-bold text-gray-900 mb-4 group-hover:text-teal-600 transition-colors">
                    {post.title}
                  </Link>
